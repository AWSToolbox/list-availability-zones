...
Functions:
    get_availability_zones: Main function to generate and return the availability zone list.
    create_boto3_client: Creates a boto3 client using the specified authentication method.
...
def query_api(client: Any, country_mapping: Dict[str, str], config: SimpleNamespace) -> List[Dict[str, Any]]:
    ...
    if 'Regions' in response:
        ...
        def process_region(region: Dict[str, Any]) -> Dict[str, Any]:
            """
            Process a single region to retrieve its availability zones and return the result.
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=config.threads) as executor:
            futures: List[concurrent.futures.Future[Dict[str, Any]]] = [executor.submit(process_region, region) for region in response['Regions']]
